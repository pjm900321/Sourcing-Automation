...
def create_driver() -> tuple[uc.Chrome, str]:
...
def cleanup_driver(driver: uc.Chrome, profile_dir: str) -> None:
    try:
        driver.quit()
    finally:
...
def human_like_scroll(driver: uc.Chrome) -> None:
...
def human_like_mouse(driver: uc.Chrome) -> None:
    try:
...
def accept_cookie_popup(driver: uc.Chrome) -> None:
    selectors = [
...
def is_access_denied(driver: uc.Chrome) -> bool:
    title = driver.title
...
def open_search_from_home(driver: uc.Chrome, keyword: str) -> None:
